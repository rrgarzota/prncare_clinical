...
foreach $schedule {
    ...
    if $terminate_loop = false {
        if ($med_intake_actual_time <= $allowed_time AND $med_intake_actual_time >= 0) {
            ...
            $previous_record_trigger_time = get top 1 med_intake_actual time from reports where ID = $get_previous_record_ID
            ...
            if ($get_previous_record_ID is blank) {
                ...
                if ($administration_type == 'single' || ($previous_record_device_ID is not blank && previous_record_device_ID == #inserted.Device_ID)) {
                    ...
                    if (($previous_record_trigger_time + 5) >= $med_intake_actual_time) {
                        $admin_status = 'correct dose'
                        $terminate_loop = true
                    } else {
